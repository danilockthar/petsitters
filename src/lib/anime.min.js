...
(function (v, p) {
	...
	function M(a) {
		return (a ? p(g.arr(a) ? a.map(w) : w(a)) : []).filter(function (
			a,
			d,
			c
		) {
	...
	function da(a, b) {
		...
		return a.tweens.map(function (c) {
			...
			c.elasticity =
				(1e3 - Math.min(Math.max(c.elasticity, 1), 999)) / 1e3;
					...
					e.tween.to.numbers.map(
						(function (a) {
							return function (b, c) {
								...
								a.round &&
									(b = Math.round(b * a.round) / a.round);
								return b;
							};
						})(e)
					),
	...
	n.remove = function (a) {
		...
		for (var b = q.length; b--; )
			for (var d = q[b], c = d.animations, e = c.length; e--; )
				F(a, c[e].animatable.target) &&
					(c.splice(e, 1), c.length || d.pause());
	};
	n.getValue = J;
	n.path = function (a, b) {
